...
1. **PR #1:** `detector_error` and `freeze_error` binary_sensors PROBLEM
   ...
   pipe risk (water hose left out in winter). Needs verification that
   either flag has ever been observed `true` in real data before PR.
2. **PR #2:** `beep`/`sensitivity` as switch+select (needs setter
